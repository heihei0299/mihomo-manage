...
| 配置应用状态 | `mihomo-manager status` | 输出 `config: applied`；校验失败输出 `config: validation-failed`；其他应用失败输出 `config: apply-failed`；reload 失败时输出 `config: pending-reload` |

### 更新流程

1. 自动备份当前 config.yaml → `config.yaml.bak.<unix-timestamp>`
2. 从远程拉取最新订阅数据
3. 以订阅数据为 base、override.yaml 为 overlay 进行合并 → 写入 config.yaml
4. 执行 reload 使新配置生效
...
| **前置** | mihomo 已安装 |
| **命令** | `mihomo-manager config override edit` |
| **通过条件** | 规则写入 `/opt/mihomo/etc/override.yaml` 并出现在生成配置中 |

### 行为

1. 打开 `$EDITOR`（默认 `vi`）编辑 `/opt/mihomo/etc/override.yaml`
2. 在 `rules:` 字段中增加兼容 mihomo 的分流规则
3. 保存后执行 `mihomo-manager subscription update`
...
| `mihomo-manager --quiet --version` | 输出当前版本，退出码 0（版本输出不受 quiet 影响） |
