...
func New(control manager.ServiceControl, lifecycle manager.LifecycleManager, config manager.ConfigManager, schedule manager.ScheduleManager, stdout, stderr io.Writer) *Handler
```
...
- The handler depends on the four focused manager role interfaces rather than a single god interface
