...
func TestSetSubscriptionSourceHonorsCancellationWhileWaiting(t *testing.T) {
	...
	m := newTestConfigManager(&fakeFileSystem{}, &fakeReleaseSource{}, &passValidator{}, noopReload, WithConfigUpdateLock(lock))
...
func TestConfigWriteOperationsPropagateLockCancellation(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			m := newTestConfigManager(&fakeFileSystem{}, &fakeReleaseSource{}, &passValidator{}, noopReload, WithConfigUpdateLock(&fakeConfigUpdateLock{err: context.Canceled}))
